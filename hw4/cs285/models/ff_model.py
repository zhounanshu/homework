...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction, _ = self(obs, acs, obs_mean, obs_std, acs_mean,  acs_std, delta_mean, delta_std)
        prediction = ptu.to_numpy(prediction)# (Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        delta_mean = ptu.from_numpy(data_statistics['delta_mean'])
        delta_std = ptu.from_numpy(data_statistics['delta_std'])
        #(Q1) compute the normalized target for the model.
        target, _ = self(obs, acs, obs_mean, obs_std, acs_mean,  acs_std, delta_mean, delta_std)
        ...
        loss = self.loss(target, next_observations)# (Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
