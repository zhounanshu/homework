...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    ob = env.reset(seed=42) # HINT: should be the output of resetting the env
    ...
    while True:
        ...
        if render:
            if hasattr(env, 'sim'):
                image_obs.append(env.sim.render(camera_name='track', height=500, width=500, mode=render_mode)[::-1])
            else:
...
def sample_n_trajectories(env, policy, ntraj, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    timesteps_this_batch = 0
    while timesteps_this_batch < max_path_length:
        path = sample_trajectory(env, policy, max_path_length, render, render_mode)
        paths.append(path)
        timesteps_this_batch += 1
        if timesteps_this_batch == ntraj:
            break
    return paths
